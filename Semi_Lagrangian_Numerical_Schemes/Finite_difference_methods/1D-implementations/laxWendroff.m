...
for i = 1:1
    ...
    for n = 1:Nt
        ...
        u_half = zeros(size(u));      
        for i = 2:Nx
            u_half(i) = u(i) - 0.5 * CFL * (u(i+1) - u(i-1)) + 0.5 * CFL^2 * (u(i+1) - 2*u(i) + u(i-1));
        end
        
    
        % Boundary conditions
        u_half(1) = u(1) - 0.5 * CFL * (u(2) - u(Nx)) + 0.5 * CFL^2 * (u(2) - 2*u(1) + u(Nx));                
        u_half(Nx+1) = u(Nx+1) - 0.5 * CFL * (u(1) - u(Nx)) + 0.5 * CFL^2 *(u(1) - 2*u(Nx+1) + u(Nx));
